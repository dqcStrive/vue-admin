<template>
  <div id="login">
    <div class="login-wrap">
      <ul class="menu-tab">
        <li v-for="item in menuTab" :key="item.name" :class="{'current': item.current}" @click="toggleMenu(item)">
          {{ item.txt }}
        </li>
      </ul>
      <!--表单 start-->
...
/**************************声明函数**********************************************88 */
      ...
      const login = (() => {
        ...
        root.$store.dispatch('app/login',repuestData).then(res => {
         //页面跳转
          root.$router.push({
            name: 'Console'
          })
